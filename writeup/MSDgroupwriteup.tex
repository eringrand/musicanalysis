...
\section{Conclusion}
After comparing the various algorithms, the simpler models gave us the best results. Our most complicated model, matrix factorization, gave us results even below the popularity baseline. On the other hand simply recommending users songs by artists they have already listened to (the artist based baseline) resulted in a significantly better recommendation system. Lastly, our song-based collaborative filtering technique produced the best results despite being a relatively simple algorithm as compared to matrix factorization. Our final results for each technique employed are summarized in the table below. 
